"use strict";

const {
  Environment,
  SignedDataVerifier,
} = require("@apple/app-store-server-library");

const APP_BUNDLE_ID = "com.levelappstudio.whocall";
const APP_APPLE_ID = 6800227705;
const CREDIT_PRODUCTS = Object.freeze({
  "com.levelappstudio.whocall.credits.3": 3,
  "com.levelappstudio.whocall.credits.5": 5,
  "com.levelappstudio.whocall.credits.10": 10,
});

// DER-encoded certificates downloaded from Apple's official PKI page:
// https://www.apple.com/certificateauthority/
const APPLE_ROOT_CERTIFICATES_BASE64 = Object.freeze([
  "",
  "",
  "",
]);

const verifierCache = new Map();

function creditAmountForProduct(productID) {
  return CREDIT_PRODUCTS[productID] || null;
}

function decodeUnverifiedEnvironment(signedTransaction) {
  if (typeof signedTransaction !== "string" ||
      signedTransaction.length < 100 || signedTransaction.length > 32_000) {
    return null;
  }
  const components = signedTransaction.split(".");
  if (components.length !== 3) return null;
  try {
    const payload = JSON.parse(Buffer.from(components[1], "base64url").toString("utf8"));
    return payload.environment === Environment.PRODUCTION ||
      payload.environment === Environment.SANDBOX ? payload.environment : null;
  } catch (_error) {
    return null;
  }
}

function verifierForEnvironment(environment) {
  if (verifierCache.has(environment)) return verifierCache.get(environment);
  const rootCertificates = APPLE_ROOT_CERTIFICATES_BASE64.map((value) =>
    Buffer.from(value, "base64"));
  const verifier = new SignedDataVerifier(
      rootCertificates,
      true,
      environment,
      APP_BUNDLE_ID,
      environment === Environment.PRODUCTION ? APP_APPLE_ID : undefined,
  );
  verifierCache.set(environment, verifier);
  return verifier;
}

async function verifyAppleCreditTransaction(
    signedTransaction,
    verifierFactory = verifierForEnvironment,
) {
  const environment = decodeUnverifiedEnvironment(signedTransaction);
  if (!environment) throw new Error("invalid-signed-transaction");

  const transaction = await verifierFactory(environment)
      .verifyAndDecodeTransaction(signedTransaction);
  const amount = creditAmountForProduct(transaction.productId);
  if (!amount || transaction.type !== "Consumable" ||
      !transaction.transactionId || !/^\d{5,64}$/.test(transaction.transactionId) ||
      (transaction.quantity ?? 1) !== 1 || transaction.revocationDate) {
    throw new Error("invalid-credit-transaction");
  }
  return {
    amount,
    environment,
    productID: transaction.productId,
    purchaseDate: Number(transaction.purchaseDate || 0),
    transactionID: transaction.transactionId,
  };
}

module.exports = {
  APP_APPLE_ID,
  APP_BUNDLE_ID,
  CREDIT_PRODUCTS,
  creditAmountForProduct,
  decodeUnverifiedEnvironment,
  verifyAppleCreditTransaction,
};
